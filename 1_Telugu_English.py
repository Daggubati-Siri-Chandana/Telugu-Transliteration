...
with open('telugu_eng.csv', mode='r',encoding='utf-8') as infile:
    reader = csv.reader(infile)
    conversiontable = {str(row[1]):str(row[0]) for row in reader}
...
def getEnglish(inputtext):
    ...
    for word in inputtext.split():
        english_output = telugu_to_eng(word)
...
print("Transliterated text in English: ",getEnglish(text))
